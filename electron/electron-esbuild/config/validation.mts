...
import { EnvConfig } from "./config.mts";
import { Target, TypeConfig } from "./enums.mts";
import type { YamlItem, YamlSkeleton } from "./yaml.mts";
import { Yaml } from "./yaml.mts";
  ...
  ensureValid(): true | never {
    ...
    if (
      ...
      const processName: keyof YamlSkeleton | undefined =
        processType === Target.main ? "mainConfig" : processType === Target.renderer ? "rendererConfig" : undefined;
      const dump =
        processType === Target.main
          ? this._dumpYaml(this.config.mainConfig, true)
          : this.config.rendererConfig !== null && processType === Target.renderer
