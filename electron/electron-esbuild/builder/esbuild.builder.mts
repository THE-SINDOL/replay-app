...
import { BaseBuilder } from "./base.builder.mts";
...
export class EsbuildBuilder extends BaseBuilder<BuildOptions> {
  ...
  listener = debounce(
    async (eventName: "add" | "addDir" | "change" | "unlink" | "unlinkDir", filePath: string, stats) => {
      this.start();
    },
    { wait: 200 },
  );
  ...
  async build(): Promise<void> {
    ...
    if (this._builder) {
      ...
      this._builder = (await esbuild.context(this._config.config)) as BuildContext;
      await this._builder.rebuild();
      await this._copyHtml();
    }

    _logger.log(this.env, "esbuild built");
  }
  ...
  async dev(start: () => void): Promise<void> {
    ...
    if (this._config.isMain) {
      ...
      const sources = path.join(path.resolve(path.dirname(this._config.config.outdir!)), "**", "*.{js,ts,tsx}");
      this._watcher = chokidar.watch(sources);
      process.on("exit", async () => {
        await this._watcher?.close();
      });
      this._watcher?.on("all", this.listener);
      //endregion
  ...
  private async _copyHtml() {
    if (this._config.fileConfig?.html) {
      const out = path.resolve(process.cwd(), this._config.fileConfig.output.dir);
