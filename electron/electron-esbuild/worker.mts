...
import { Config, Item } from "./config/config.mts";
import type { Configurator } from "./config/configurators/base.configurator.mts";
import type { TypeConfig } from "./config/enums.mts";
import { Target } from "./config/enums.mts";
import type { PossibleConfiguration } from "./config/types.mts";
import { configByEnv } from "./config/utils.mts";
import { ConfigFile } from "./config/validation.mts";
import type { Yaml, YamlSkeleton } from "./config/yaml.mts";
import { Logger } from "./console.mts";
import type { Env } from "./env.mts";

const _outMain = "out_main.mts";
const _outRenderer = "out_renderer.mts";
...
const _buildUserConfig = async <C extends PossibleConfiguration>(configPath: string, target: Target): Promise<C> => {
...
const _loadYaml = (file: string): Yaml => {
  ...
  const configFile = new ConfigFile(yaml.load(fileContent) as unknown as YamlSkeleton);
...
class _WorkerConfigurator {
  ...
  constructor({ main, renderer }: { main: Configurator<TypeConfig>; renderer: Configurator<TypeConfig> | null }) {
    this.main = main;
...
export class Worker<M extends PossibleConfiguration, R extends PossibleConfiguration> {
  ...
  static fromFile<M extends PossibleConfiguration, R extends PossibleConfiguration>({
    file,
    env,
  }: {
    file: string;
  ...
  async toConfigAsync(): Promise<Config<M, R>> {
    ...
    const rendererConfigPath = rendererConfig !== null ? _resolve(rendererConfig.path) : null;

    const userMainConfig = await _buildUserConfig<M>(mainConfigPath, Target.main);
    ...
    mainConfigFinal = deepMerge(
      this.configurator.main.toBuilderConfig(this._main, mainConfigFinal, Target.main) as Partial<M>,
      mainConfigFinal,
      { clone: false },
    );
